...
class Menulunchcontroller extends Controller
{
    ...
    public function form(){
        return view("admin.pages.menu-lunch");
    }
    ...
    public function get_data(){
        ...
        return view("admin.pages.menu-lunch-data" , compact('data'));
       }

       public function edit(Request $request){
        $data = Menulunchmodel::where("id",$request->id )->first();
        return view('admin.pages.menu-lunch-edit',compact('data'));
    }
    public function update(Request $request){
        ...
        if ($request->file('menu_image')) {
            ...
            if(public_path('Lunch'). '/' .$data->menu_image){
                unlink(public_path('Lunch'). '/' .$data->menu_image);
           } 
            $file  = $request->file('menu_image');
            $exe = $file->extension();
            $filename = rand(1,1000). '.' .$exe;
        ...
        $data->update();
        return redirect()->route('menu-lunch-data');
    }
}
